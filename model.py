...
def hash_feature_ids(
    ...
    """Map raw feature ids to hash-bucket indices; padding id 0 stays 0."""
    raw = raw_ids.long()
    mask = raw > 0
    hashed = (
        raw * 1315423911 + (slot_index + 1) * 2654435761
    ) % bucket_size + 1
    return hashed * mask.long()
...
class MultiSeqHyFormerBlock(nn.Module):
    ...
    def __init__(
        ...
        if len(seq_encoder_types) != num_sequences:
            raise ValueError(
                f"seq_encoder_types length {len(seq_encoder_types)} != "
...
class UserPairEncoder(nn.Module):
    ...
    def __init__(
        ...
        use_hash_cross: bool = False,
        hash_bucket_size: int = 200000,
        hash_top_k: int = 3,
        dropout: float = 0.0,
    ) -> None:
        super().__init__()
        ...
        self.item_aware = item_aware or time_aware
        self.time_aware = time_aware
        self.use_hash_cross = use_hash_cross
        self.hash_bucket_size = int(hash_bucket_size)
        self.hash_top_k = int(hash_top_k)

        self.id_embs = nn.ModuleList([
            nn.Embedding(int(spec['vocab_size']) + 1, pair_emb_dim, padding_idx=0)
            for spec in specs
        ])
        if use_rank:
            self.rank_embs = nn.ModuleList([
                nn.Embedding(int(spec['length']) + 1, pair_emb_dim, padding_idx=0)
                for spec in specs
            ])
        else:
            self.rank_embs = nn.ModuleList()
        ...
        self.profile_proj = nn.Sequential(
            nn.Linear(len(specs) * pair_emb_dim, d_model),
            nn.LayerNorm(d_model),
            nn.SiLU(),
            nn.Dropout(dropout),
        )
        ...
        if self.item_aware:
            query_dim = d_model * (2 if time_aware else 1)
            self.item_query_proj = nn.Linear(query_dim, pair_emb_dim)
            self.item_context_proj = nn.Linear(d_model, pair_emb_dim)
            self.item_aware_proj = nn.Sequential(
                nn.Linear(pair_emb_dim * 3, d_model),
                nn.LayerNorm(d_model),
                nn.SiLU(),
                nn.Dropout(dropout),
            )
    ...
    def _field_slots(
        ...
        return ids, log_weights, mask, slot_emb * mask.unsqueeze(-1).float(), attn

    def _hash_cross_token(
        self,
        field_ids: List[torch.Tensor],
        field_log_weights: List[torch.Tensor],
        field_masks: List[torch.Tensor],
    ) -> torch.Tensor:
        cross_vecs = []
        for i in range(len(field_ids) - 1):
            left_ids = field_ids[i][:, :self.hash_top_k]
            right_ids = field_ids[i + 1][:, :self.hash_top_k]
            left_mask = field_masks[i][:, :self.hash_top_k]
            right_mask = field_masks[i + 1][:, :self.hash_top_k]
            left_w = field_log_weights[i][:, :self.hash_top_k]
            right_w = field_log_weights[i + 1][:, :self.hash_top_k]

            cross_mask = left_mask.unsqueeze(2) & right_mask.unsqueeze(1)
            cross_ids = (
                left_ids.unsqueeze(2) * 1315423911
                + right_ids.unsqueeze(1) * 2654435761
                + (i + 1) * 104729
            ) % self.hash_bucket_size + 1
            cross_ids = cross_ids.long() * cross_mask.long()
            cross_emb = self.hash_emb(cross_ids)

            cross_scores = (left_w.unsqueeze(2) + right_w.unsqueeze(1)).masked_fill(
                ~cross_mask, -1.0e4)
            cross_attn = torch.softmax(cross_scores.flatten(1), dim=1).view_as(cross_scores)
            cross_attn = cross_attn * cross_mask.float()
            cross_vec = (cross_emb * cross_attn.unsqueeze(-1)).sum(dim=(1, 2))
    ...
    def forward(
        ...
        user_int_feats: torch.Tensor,
        user_dense_feats: torch.Tensor,
        item_ns: Optional[torch.Tensor] = None,
        sample_time_token: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        fid_vecs = []
        field_ids = []
        field_log_weights = []
        ...
        for i, spec in enumerate(self.specs):
            rank_emb = self.rank_embs[i] if self.use_rank else None
            ids, log_weights, mask, slot_emb, attn = self._field_slots(
                user_int_feats, user_dense_feats, spec, self.id_embs[i], rank_emb)
            fid_vec = (slot_emb * attn.unsqueeze(-1)).sum(dim=1)
            fid_vecs.append(fid_vec)
            field_ids.append(ids)
        ...
        if self.item_aware:
            if item_ns is None:
                item_pool = fid_stack.new_zeros(fid_stack.shape[0], self.d_model)
            else:
                item_pool = item_ns.mean(dim=1)
            if self.time_aware and sample_time_token is not None:
                query_input = torch.cat([item_pool, sample_time_token], dim=-1)
            elif self.time_aware:
                query_input = torch.cat([item_pool, torch.zeros_like(item_pool)], dim=-1)
            else:
            ...
            item_token = self.item_aware_proj(torch.cat([
                item_pair_vec,
                item_ctx,
        ...
        if self.use_hash_cross:
            tokens.append(self._hash_cross_token(
                field_ids, field_log_weights, field_masks).unsqueeze(1))
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        seq_use_hash_emb: bool = True,
        seq_hash_bucket_size: int = 200000,
        ...
        self.seq_use_hash_emb = seq_use_hash_emb
        ...
        if self.has_user_pair:
            self.user_pair_encoder = UserPairEncoder(
                ...
                time_aware=user_pair_time_aware,
                use_hash_cross=user_pair_use_hash_cross,
                hash_bucket_size=user_pair_hash_bucket_size,
                hash_top_k=user_pair_hash_top_k,
        ...
        def _make_seq_embs(vocab_sizes):
            ...
            for i, vs in enumerate(vocab_sizes):
                ...
                if skip and seq_use_hash_emb and int(vs) > 0:
        ...
        if seq_encoder_types is None:
            seq_encoder_types = [seq_encoder_type] * self.num_sequences
        if len(seq_encoder_types) != self.num_sequences:
            raise ValueError(
                f"seq_encoder_types length {len(seq_encoder_types)} != "
                f"num_sequences {self.num_sequences}"
            )
        logging.info(f"Per-domain seq encoders: {dict(zip(self.seq_domains, seq_encoder_types))}")

        if seq_use_hash_emb:
            self.seq_hash_emb = nn.Embedding(
                self.seq_hash_bucket_size + 1, emb_dim, padding_idx=0
            )
        else:
            self.seq_hash_emb = None
        ...
        for domain in self.seq_domains:
            ...
            embs, idx_map, is_id, is_hash = _make_seq_embs(vs)
            ...
            hash_count = sum(is_hash)
            if hash_count > 0:
                logging.info(
                    f"seq hash embedding: {domain} uses hash for {hash_count}/{len(vs)} "
                    f"skipped side features (bucket_size={self.seq_hash_bucket_size})"
                )
        ...
        self.blocks = nn.ModuleList([
            MultiSeqHyFormerBlock(
                ...
                seq_encoder_types=seq_encoder_types,
                hidden_mult=hidden_mult,
                dropout=dropout_rate,
                top_k=seq_top_k,
                causal=seq_causal,
    ...
    def _init_params(self) -> None:
        ...
        if self.seq_hash_emb is not None:
            nn.init.xavier_normal_(self.seq_hash_emb.weight.data)
            self.seq_hash_emb.weight.data[0, :] = 0

        if self.has_user_pair:
            for emb in self.user_pair_encoder.id_embs:
                nn.init.xavier_normal_(emb.weight.data)
                emb.weight.data[0, :] = 0
            for emb in self.user_pair_encoder.rank_embs:
                nn.init.xavier_normal_(emb.weight.data)
                emb.weight.data[0, :] = 0
            if self.user_pair_encoder.use_hash_cross:
                nn.init.xavier_normal_(self.user_pair_encoder.hash_emb.weight.data)
                self.user_pair_encoder.hash_emb.weight.data[0, :] = 0
    ...
    def _embed_seq_domain(
        ...
        for i in range(S):
            ...
            if real_idx == -1 and is_hash[i] and self.seq_hash_emb is not None:
                raw_ids = seq[:, i, :].long()
                hash_ids = hash_feature_ids(
                    raw_ids, slot_index=i, bucket_size=self.seq_hash_bucket_size
                )
                e = self.seq_hash_emb(hash_ids)
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        for domain in self.seq_domains:
            tokens = self._embed_seq_domain(
                ...
                self._seq_is_id[domain], self._seq_is_hash[domain],
