...
def parse_args() -> argparse.Namespace:
    ...
    # User pair: aligned user_int / user_dense fids (default 62-66).
    parser.add_argument('--use_user_pair', action='store_true', default=False,
                        help='Enable UserPairEncoder for aligned int/dense fids')
    parser.add_argument('--user_pair_fids', type=str, default='62,63,64,65,66',
                        help='Comma-separated user fids for pair encoder')
    parser.add_argument('--user_pair_emb_dim', type=int, default=32,
                        help='Internal embedding dim in UserPairEncoder')
    parser.add_argument('--user_pair_use_rank', action='store_true', default=True,
                        help='Add per-slot rank embeddings in UserPairEncoder')
    parser.add_argument('--no_user_pair_rank', dest='user_pair_use_rank',
                        action='store_false',
                        help='Disable rank embeddings in UserPairEncoder')
    parser.add_argument('--user_pair_item_aware', action='store_true', default=True,
                        help='Add item-aware user pair NS token')
    parser.add_argument('--no_user_pair_item_aware', dest='user_pair_item_aware',
                        action='store_false',
                        help='Disable item-aware user pair token')
    parser.add_argument('--user_pair_time_aware', action='store_true', default=True,
                        help='Condition item-aware pair token on discrete time_feats')
    parser.add_argument('--no_user_pair_time_aware', dest='user_pair_time_aware',
                        action='store_false',
                        help='Disable time conditioning for item-aware pair token')
    parser.add_argument('--user_pair_use_hash_cross', action='store_true', default=False,
                        help='Add optional hash-cross user pair token')
    parser.add_argument('--user_pair_hash_bucket_size', type=int, default=200000,
                        help='Hash bucket size for user_pair_use_hash_cross')
    parser.add_argument('--user_pair_hash_top_k', type=int, default=3,
                        help='Top-k slots per fid for hash cross')
...
def main() -> None:
    ...
    if (model_args["rank_mixer_mode"] == 'full'
        ...
        logging.warning(
            f"rank_mixer_mode=full requires d_model % T == 0, but "
            f"d_model={args.d_model}, T={estimated_T} after user pair tokens. "
            f"Falling back to rank_mixer_mode=ffn_only."
        )
    ...
    trainer = PCVRHyFormerRankingTrainer(
        ...
        train_config={**vars(args), "user_pair_specs": user_pair_specs},
    )
